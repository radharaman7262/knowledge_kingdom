...
@section('slider_page_active','active open')
@section('content')
                  ...
                  <table class="table " id="myDataTable">
                    ...
                    @foreach($sliderdata as $rows)
                      <tr>
						...
						<td>{{$rows->slider_type}}</td>

                        <td>  <div class="form-check form-switch mb-2">
                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault"  @if($rows->status == 1) checked @endif onclick="updateStatus({{$rows->id}},{{$rows->status}},'sliders','status')" />

                      </div></td>

                        <td><a href="{{url('admin/edit/slider/'.$rows->id)}}" class="btn btn-info"><i class='bx bx-edit'></i></a>&nbsp;<a class="btn btn-danger" href="#" onclick= deleteRecord("{{route('admin.delete.slider',$rows->id)}}")><i class='bx bx-message-square-x' ></i></a></td>
                      </tr>
                      @endforeach
